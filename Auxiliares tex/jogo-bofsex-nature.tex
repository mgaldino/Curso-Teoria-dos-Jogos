...
\usepackage{Sweave}
\begin{document}
\input{jogo-bofsex-nature-concordance}
...
\node[hollow node, label=above:{Natureza}] (natureza) {}
  child {node[solid node] (serena1) {}
    child{node[solid node, label={[label distance=-1mm, xshift=-2mm]left:Nina}] (nina1) {}
      child{
        node[solid node, label=below:{$3,2$}]{}
        edge from parent
        node[left, yshift=3] {B}
      }
      child{
        node[solid node, label=below:{$1,1$}]{}
        edge from parent
        node[right, yshift=3] {S}
      }
      edge from parent
      node[left, yshift=-3, xshift = -5] {Bach}
    }
    child{node[solid node, label={[label distance=-1mm, xshift=-2mm]left:Nina}] (nina2) {}
      child{
  ...
  child {node[solid node] (serena2) {}
    child{node[solid node, label={[label distance=-1mm, xshift=2mm]right:Nina}] (nina3) {}
      child{
    ...
    child{node[solid node, label={[label distance=-1mm, xshift=2mm]right:Nina}] (nina4) {}
      child{
        node[solid node, label=below:{$2,2$}]{}
        edge from parent
        node[left, yshift=3] {B}
