...
Para construir a árvore do jogo, precisamos entender os payoffs do status quo para cada jogador. Já sabemos que na posição ideal do presidente, ele obtém utilidade de 2. Nesse caso, Câmara tem um payoff de -2 e Senado de -1. Isso pode ser representado pela seguinte posição na reta.

```{r results = "asis", echo=FALSE}
library(ggplot2)

# Crie um dataframe com as informações dos jogadores
jogadores <- data.frame(
  jogador = c("Jogador 1", "Jogador 2", "Jogador 3"),
  reta = rep(0,3),
  ponto_ideal = c(-2, 1, 2)
)

# Crie o gráfico utilizando o ggplot2
ggplot(jogadores, aes(y = reta, x = ponto_ideal)) +
  geom_point(size = 4, color = "blue") +  # pontos representando os pontos ideais
  geom_line(color = "gray") +  # linha conectando os pontos
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # linha horizontal em y = 0
  labs(title = "Preferências Espaciais dos Jogadores", x = "Jogador", y = "Posição Ideal")  # títulos dos eixos e do gráfico
```


1. Quando o SQ é -3 de payoff para o presidente, a Câmara obtém um payoff de 1 (ideal gera payoff de 2, SQ gera payoff 3 para a Câmara, perde 1 de utilidade em relação ao ideal) e o Senado obtém payoff de 0 (ideal é 1, SQ é 3, então perde 2 de utilidade)

```{r results = "asis", echo=FALSE}
library(rgamer)

gamea <- extensive_form(
  players = list("Camara",
                 c(rep("Senado", 2), NA),
                 c(NA, "Camara", rep(NA, 2), "Camara", NA),
                 rep(NA, 4)),
...
Logo, o equilíbrio é o mesmo de antes, rejeita.
...
gameb <- extensive_form(
  ...
  actions = list(c("A pl0", "E pl0", "Rejeita"), # segundo
                 ...
                 c("A pl-S", "A pl0"),
                 c("Aprova pl-EC-ES", "A pl0")),
  payoffs = list(Camara = c(1, -2, 1, 1, 1 ,1, -2, 1, -2),
                 Senado =  c(0, -1, 0, 1 ,0 ,1, -1, 1, -1)),
  direction = "horizontal")
...
gamec <- extensive_form(
  ...
  actions = list(c("A pl0", "E pl0", "Rejeita"), # segundo
                 ...
                 c("A pl-S", "A pl0"),
                 c("Aprova pl-EC-ES", "A pl0")),
  payoffs = list(Camara = c(0, -2, 0, 1, 0 ,1, -2, 1, -2),
